...
<template>
 ...
 <v-btn @click="openDialog" color="primary">Add Security Control</v-btn>
...
export default {
  ...
  data() {
    return {
      teams: [],
      userHeaders: [
        { title: 'Security Control', align: 'start', value: 'name' },
  ...
  methods: {
    ...
    async submitData(inputSecControlCon,inputRisk) {
      try {

        const response = await this.postData(inputSecControlCon,inputRisk); // Awaiting the POST request response
